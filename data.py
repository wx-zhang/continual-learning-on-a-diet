...
def get_loader(train_set, batch_size, args, is_train=True):
    ...
    if is_train:
        train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=(
            train_sampler is None), num_workers=args.workers, pin_memory=False, sampler=train_sampler, drop_last=True)
    else:
...
def update_buffer(args, task, cur_set):
    ...
    """
    if task > 0:
        last_memory = torch.load(
            f"{args.output_dir}/buffer.torchSave").to(args.rank)
        prev_len = last_memory.size(1)
    else:
        prev_len = -1
    cur_len = len(cur_set)

    mem_len = min([item for item in [args.replay_buffer_size //
                  (task+1), cur_len, prev_len] if item > 0])
