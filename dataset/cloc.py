...
class CLOC(object):
    def __init__(self, args):
        ...
        self.train_data = torch.load(
            f"{self.data_root}/train_store_loc.torchSave")
        self.train_label = torch.load(
            f"{self.data_root}/train_labels.torchSave")
        self.test_data = pd.read_csv(f"{self.data_root}/yfcc100m_metadata_with_labels_usedDataRatio0.05_t110000_t250_valid_files_2004To2014_compact_val.csv",
                                     header=None, names=["class_id", "date", 'time', 'cell', 'path'])
