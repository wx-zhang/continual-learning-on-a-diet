...
class trans_label(object):
    ...
    def __init__(self, task, path):
        ...
        self.pre_classes = sum([len(os.listdir(f'{path}/{i}/val/')) for i in range(task)])
...
class ImageNet(object):
    ...
    def get_labeled_set(self,task):
        ...
        data =  datasets.ImageFolder(cur_folder, self.train_transform, target_transform=trans_label(task, self.path))
        label_index = torch.load(f'{self.path}/{task}/label_index-{self.label_rate}.torchSave')
    ...
    def get_unlabeled_set(self, task):
        ...
        unlabeled_index = torch.load(f'{self.path}/{task}/unlabeled_index-{self.label_rate}.torchSave')
    ...
    def get_eval_set(self, task, per_task_eval):
        if per_task_eval:
            ...
            valset = datasets.ImageFolder(cur_folder, self.val_transform, target_transform=trans_label(task, self.path))
            return valset
        else:
            data = []
            for i in range(task+1):
                cur_folder = f"{self.path}/{i}/val/"
                data.append(datasets.ImageFolder(cur_folder, self.val_transform, target_transform=trans_label(i, self.path)))
